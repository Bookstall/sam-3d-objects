...
class Sam3Utils:
    ...
    def sam3_with_points(
        ...
        Args:
            ...
            input_points_labels: 每个点的标签，1=正向点，0=负向点
            point_radius: 点转成小框时的半径（像素）
        ...
        boxes = [
            [
                max(0, x - point_radius),
                max(0, y - point_radius),
                min(w, x + point_radius),
                min(h, y + point_radius),
            ]
            for x, y in input_points
        ]
        input_boxes = [boxes]
        input_boxes_labels = [input_points_labels]
        inputs = self.sam3_processor(
            images=image,
            input_boxes=input_boxes,
            input_boxes_labels=input_boxes_labels,
        ...
        if len(results["masks"]) == 0: # 如果没有任何实例掩码，则返回全透明掩码
        ...
        overlay = self.overlay_masks_new(image=image, masks=results["masks"])
        # 取第一个实例 mask 用于 3D
        first_mask = results["masks"][0].cpu().numpy()
        mask_np = (255 * (first_mask > 0.5)).astype(np.uint8)
        return overlay, mask_np


    def sam3_with_points_new(
        self,
        image: Union[Image.Image, str, np.ndarray],
        input_points: List[List[int]],
